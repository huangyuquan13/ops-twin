...
### [▶] Stage 3: 任务中心 (自动化演练引擎 - 当前进行中)
- [x] **预案方案库**: task_plan 表建立，完整 CRUD 接口与前端管理页面已上线。
- [x] **演练工作流**: 异步调度引擎（TaskRecordController.trigger + @Async 引擎），解析 steps_json 逐步模拟执行，实时更新 task_record 状态。
- [x] **实时监控终端**: 黑底绿字 Xterm 风格控制台，WebSocket 实时推流演练日志，含进度条/状态徽章/语义着色。
