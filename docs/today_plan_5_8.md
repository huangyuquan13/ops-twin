...
### 任务四：演练编排工作站 (Workflow Designer) - 额外攻坚
- **需求**: 激活 `tasks/workflow.vue`，告别手写 JSON。
- **细节**:
  - **画布引入**: 集成 Vue Flow，提供“开始”、“动作节点”、“结束”三类组件。
  - **节点配置**: 点击动作节点可配置 `action`（停止/重启等）和 `target`（主机名）。
  - **自动序列化**: 将画布上的连线关系自动转换为后端引擎可识别的 `steps_json`。
  - **预案联动**: 从方案库跳转至此处进行深度编排。
