...
//响应式获取时候 kpiList[0].value
//而ref得 kpiList.value[0].value
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      const option = {
        ...
        tooltip: { trigger: "axis" }, //悬浮提示，axis 模式
        xAxis: {
          type: "category", //非连续性轴
          data: res.data.xAxis,
        ...
        yAxis: {
          type: "value", //数值轴
          axisLabel: { formatter: "{value} %" }, //标签格式化为%
...
const initPieChart = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      const option = {
        backgroundColor: "transparent",
        tooltip: { trigger: "item" }, //数据项展示
        legend: { top: "5%", left: "center" }, //图列位置
        series: [
          {
            name: "资产分类",
            type: "pie",
