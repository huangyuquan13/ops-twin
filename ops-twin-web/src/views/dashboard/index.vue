  ...
  <div class="dashboard-container">
    ...
    <div class="overlay-stats">
      <div class="stat-item">
        ...
        <div class="value">{{ total }}</div> <!-- //计数器 用于顶部状态栏 -->
      </div>
      <div class="stat-item">
      ...
      <div class="stat-item clickable" @click="toggleThermalMode">
        ...
        <div class="value" :style="{ color: isThermalMode ? '#ff4d4f' : '#409eff' }">
    ...
    <div class="view-controls">
      <el-button type="primary" size="large" @click="resetPerspective" round shadow>
        重置视角
      </el-button>
      <Transition name="fade">
        <el-button v-if="currentView !== 'L1'" type="warning" size="large" @click="goBack" round shadow style="margin-left: 15px">
    ...
    <Transition name="fade">
      <div v-if="selectedCabinet && currentView === 'L2'" class="info-card">
        ...
        <p><span>机柜位置:</span> [X: {{ selectedCabinet.posX }}, Z: {{ selectedCabinet.posZ }}]</p>
        <div class="divider"></div>
        <p>提示：现在您可以直接点击机柜内部发光的刀片服务器，进入 L3 硬件剖解层级！</p>
      </div>
    </Transition>
      ...
      >
        ...
        <div class="holo-body">
          <div class="data-row">
            ...
            <span class="value" style="color: #fff">{{ activeBladeData?.parentHost?.hostname }}</span>
          </div>
          <div class="data-row">
            <span class="label">物理插槽 (U位)</span>
            <span class="value val-blue">{{ activeBladeData?.slotIndex !== undefined ? activeBladeData.slotIndex + 1 : '' }}U</span>
          </div>
          <div class="data-row">
            <span class="label">实时 CPU 负载</span>
            <span class="value" :class="(activeBladeData?.usage || 0) > 80 ? 'val-red' : 'val-green'">
              {{ activeBladeData?.usage || 0 }}%
            </span>
          </div>
          <div class="data-row">
            <span class="label">风扇转速 (RPM)</span>
            <span class="value val-yellow">{{ 3000 + (activeBladeData?.usage || 0) * 20 }} RPM</span>
          </div>
          <div class="data-row">
            <span class="label">磁盘 I/O (MB/s)</span>
            <span class="value val-blue">{{ ((activeBladeData?.usage || 0) * 1.5).toFixed(1) }} MB/s</span>
          </div>
        </div>
        <!-- 模拟风扇旋转动画 -->
        <div class="fan-animation" :style="{ animationDuration: `${100 / ((activeBladeData?.usage || 0) + 10)}s` }">
...
const fetchAndRenderAssets = async () => {
  try {
    const res: any = await request.get("/api/asset/host/list", { params: { current: 1, size: 100 } });
    if (res.code === 200) {
...
const createCabinetModel = (cabinetId: string, posX: number, posZ: number, hosts: any[]) => {
  ...
  const lineMat = new THREE.LineBasicMaterial({ color: 0x00e5ff, transparent: true, opacity: 0.5 });
...
const renderHostModels = () => {
  const oldModels = scene.children.filter((child) => child.name === "hostModel");
  oldModels.forEach((model) => scene.remove(model));

  // 【核心重构】：根据 cabinet_id 进行聚合，同一个 cabinet_id 的服务器放到一个机柜里
  const cabinets = new Map<string, any[]>();
  hostList.value.forEach((host) => {
    // 兼容驼峰和下划线
    const cabId = host.cabinetId || host.cabinet_id || '未知机柜';
    if (!cabinets.has(cabId)) {
      cabinets.set(cabId, []);
    }
    cabinets.get(cabId)!.push(host);
  });

  // 遍历聚合后的机柜，进行 3D 渲染
  cabinets.forEach((hostsInCabinet, cabId) => {
    // 整个机柜的坐标以它里面的第一台主机的坐标为准
    const baseHost = hostsInCabinet[0];
    const posX = baseHost.posX || baseHost.pos_x || 0;
    const posZ = baseHost.posZ || baseHost.pos_z || 0;
    const cabinet = createCabinetModel(cabId, posX, posZ, hostsInCabinet);
    scene.add(cabinet);
  });
};
  ...
  if (currentView.value === 'L1') {
    for (let i = 0; i < intersects.length; i++) {
      ...
      if (current) {
        ...
        // 瞬间将相机传送到机柜前方，去掉多余的 y 轴偏移，z 轴拉远到 3.2 保证完整显示
        camera.position.set(current.position.x, current.position.y, current.position.z + 3.2);
        controls.target.set(current.position.x, current.position.y, current.position.z);
        controls.update();
...
const resetPerspective = () => {
  if (currentView.value === 'L1') {
    ...
    controls.update();
  } else if ((currentView.value === 'L2' || currentView.value === 'L3') && activeCabinet) {
    // 重置 L2/L3 怼脸视角
    camera.position.set(activeCabinet.position.x, activeCabinet.position.y, activeCabinet.position.z + 3.2);
    controls.target.set(activeCabinet.position.x, activeCabinet.position.y, activeCabinet.position.z);
    controls.update();
  }
};
    ...
    if (obj.name === 'hostModel') {
      ...
      servers.forEach((server: any) => {
        ...
        if (isThermalMode.value) {
          const heatColor = new THREE.Color().setHSL((100 - usage) / 360, 1, 0.5);
...
const handleResize = () => { //监听窗口变化 确保画布不拉伸
  if (!threeContainer.value) return;
...
<style scoped>
.dashboard-container { position: relative; width: 100%; height: calc(100vh - 80px); background: #020508; overflow: hidden; }
.three-canvas-wrapper { width: 100%; height: 100%; }

.overlay-stats { position: absolute; top: 20px; left: 20px; display: flex; gap: 15px; z-index: 10; }
.stat-item { background: rgba(5, 12, 20, 0.85); border: 1px solid rgba(0, 229, 255, 0.2); padding: 12px 20px; border-radius: 4px; backdrop-filter: blur(8px); }
.stat-item.clickable { cursor: pointer; transition: all 0.3s; }
.stat-item.clickable:hover { border-color: #00e5ff; background: rgba(0, 229, 255, 0.1); }
.stat-item .label { color: #666; font-size: 11px; text-transform: uppercase; margin-bottom: 4px; }
.stat-item .value { color: #00e5ff; font-size: 20px; font-weight: bold; font-family: "JetBrains Mono"; }

.view-controls { position: absolute; bottom: 40px; left: 50%; transform: translateX(-50%); z-index: 20; }

.info-card {
  position: absolute; top: 100px; right: 20px; width: 320px;
  background: rgba(5, 12, 20, 0.9); border: 1px solid #00e5ff; padding: 24px;
  border-radius: 2px; color: #fff; z-index: 100; box-shadow: 0 0 40px rgba(0, 229, 255, 0.15);
}
.card-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; }
.card-header h3 { margin: 0; font-size: 20px; color: #00e5ff; }
.info-card p { margin: 12px 0; font-size: 13px; line-height: 1.6; }
.info-card p span { color: #666; width: 80px; display: inline-block; }
.divider { height: 1px; background: rgba(0, 229, 255, 0.1); margin: 20px 0; }
...
.val-green { color: #52c41a; font-weight: bold; text-shadow: 0 0 10px #52c41a; }
.val-red { color: #ff4d4f; font-weight: bold; text-shadow: 0 0 10px #ff4d4f; }
.val-yellow { color: #faad14; }
.val-blue { color: #409eff; }

/* CSS 风扇动画特效 */
.fan-animation { position: absolute; top: 30px; right: 30px; width: 40px; height: 40px; animation: spin linear infinite; opacity: 0.5; }
.fan-blade { position: absolute; top: 50%; left: 50%; width: 20px; height: 4px; background: #00e5ff; transform-origin: 0 50%; border-radius: 2px; }
@keyframes spin { 100% { transform: rotate(360deg); } }

:deep(.host-label) { color: #00e5ff; font-size: 12px; padding: 4px 10px; background: rgba(0, 229, 255, 0.1); border: 1px solid rgba(0, 229, 255, 0.5); border-radius: 2px; white-space: nowrap; pointer-events: none; }
